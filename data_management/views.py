...
def filter_form(input_dict):
    ...
    for proc in input_dict.keys():
        ...
        if proc == "KOHEtch":
            q_obj = (proc, KOHEtch.objects.filter(query).order_by('{0}_step_time'.format(proc)))
        if proc == "NickelPlating":
            q_obj = (proc, NickelPlating.objects.filter(query).order_by('{0}_step_time'.format(proc)))
        if proc == "Patterning":
